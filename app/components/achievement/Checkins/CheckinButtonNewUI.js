...
const checkIcon = (
  <Icon name="check" size={responsiveFontSize(1.5)} color="#8A1E41" />
);
const checkedIcon = (
  <Icon name="check" size={responsiveFontSize(1.5)} color="#FFF" />
);
/**
...
export class EventCheckinButtonX extends React.PureComponent {
  ...
  render() {
    let checkedInBtn = (
      <View style={[styles.bigSchedule,{backgroundColor:"#8A1E41",paddingHorizontal: responsiveWidth(4),height:responsiveHeight(3.7)}]}>
        <Text style={[styles[this.props.style + "text"],{color:"#FFF"}]}>
          CHECKED-IN
        </Text>
      </View>
    );
    ...
    if (this.props.from === "schedule") {
      myCheckinButton = (
        <View style={[styles.bigSchedule,{paddingHorizontal: responsiveWidth(4)}]}>
    ...
    } else if (this.props.checkinStatus) {
      return (
        <View>
            <Analytics ref="analytics" />
            {checkedInBtn}
